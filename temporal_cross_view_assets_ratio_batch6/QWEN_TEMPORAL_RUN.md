...
3. Extract each unique third-person sampled frame from the generated temporal
   contact sheets.
4. Ask Qwen to verify the source identity against every target frame.
5. Build independent occurrence spans for each case.
6. Deterministically reject any candidate window that:
   - is not fully inside a confirmed occurrence span;
   - is not a generated 20%-30% window;
   - confirms the target in fewer than 75% of sampled frames;
   - has more than two consecutive absent samples; or
   - lacks evidence near either endpoint.
7. Select the highest whole-window score or emit `uncertain`.
8. Rewrite `temporal_analysis_result.json`, render the comparison image, and
...
```text
/scratch/users/ntu/gwang016/qwen35-4b/temporal_batch6_work/
```
...
python qwen_temporal_runner.py \
  --assets-root "$PWD" \
  --work-dir "$HOME/scratch/qwen35-4b/temporal_batch6_work" \
  --render-only
```
